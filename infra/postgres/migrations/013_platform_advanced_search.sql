...
CREATE TABLE IF NOT EXISTS odf.platform_matching_evaluations (
  ...
  threshold double precision NOT NULL CHECK (isfinite(threshold) AND threshold >= 0 AND threshold <= 1),
...
CREATE TABLE IF NOT EXISTS odf.platform_spatial_asset_links (
  ...
  confidence double precision NOT NULL CHECK (isfinite(confidence) AND confidence >= 0 AND confidence <= 1),
